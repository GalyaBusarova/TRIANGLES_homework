...
std::vector<int> find_answer(const std::unordered_map<int, std::vector<point_t<float>>> &hash_of_triangles, int N)
{
    ...
    for (int i = 0; i < N; i++)
    {
        vect_t<float> first_vec_i = hash_of_triangles.at(i)[0].create_vector_between(hash_of_triangles.at(i)[1]);
        vect_t<float> second_vec_i = hash_of_triangles.at(i)[1].create_vector_between(hash_of_triangles.at(i)[2]);
        vect_t<float> third_vec_i = hash_of_triangles.at(i)[2].create_vector_between(hash_of_triangles.at(i)[0]);
        ...
        if (!is_valid_triangle(hash_of_triangles.at(i))) 
        {
            continue;
        }
        ...
        for (int j = i + 1; j < N; j++)
        {
            vect_t<float> first_vec_j = hash_of_triangles.at(j)[0].create_vector_between(hash_of_triangles.at(j)[1]);
            vect_t<float> second_vec_j = hash_of_triangles.at(j)[1].create_vector_between(hash_of_triangles.at(j)[2]);
            vect_t<float> third_vec_j = hash_of_triangles.at(j)[2].create_vector_between(hash_of_triangles.at(j)[0]);
            ...
            if (!is_valid_triangle(hash_of_triangles.at(j)))
            {
                continue;
            }
            ...
            Parallel_Or_Not status = check_if_parallel(normal_vec_i, normal_vec_j, hash_of_triangles.at(j)[0], hash_of_triangles.at(i)[0]);
            ...
            point_t<float> refer_point_j = hash_of_triangles.at(j)[0];

            for (int k = 0; k < 3; k++)
            {
                if (normal_vec_j.scalar_compos(refer_point_j.create_vector_between(hash_of_triangles.at(i)[k])) < 0){summ_i--;}
                if (normal_vec_j.scalar_compos(refer_point_j.create_vector_between(hash_of_triangles.at(i)[k])) > 0){summ_i++;}
            }
            ...
            point_t<float> refer_point_i = hash_of_triangles.at(i)[0];

            for (int k = 0; k < 3; k++)
            {
                if (normal_vec_i.scalar_compos(refer_point_i.create_vector_between(hash_of_triangles.at(j)[k])) < 0){summ_j--;}
                if (normal_vec_i.scalar_compos(refer_point_i.create_vector_between(hash_of_triangles.at(j)[k])) > 0){summ_j++;}
            }
                        ...
                        int tri_i_index, int tri_j_index)  
{
    for (int edge_i = 0; edge_i < 3; edge_i++) {
        for (int edge_j = 0; edge_j < 3; edge_j++) {
            ...
            for (int k = 0; k < 3; k++) {
                float proj = hash_of_triangles.at(tri_i_index)[k].project_point(normalized_axis);
            ...
            for (int k = 0; k < 3; k++) {
                float proj = hash_of_triangles.at(tri_j_index)[k].project_point(normalized_axis);
                        ...
                        const vect_t<float>& normal_vect)
{
    for (int i = 0; i < 3; i++)
    {
        ...
        for (int k = 0; k < 3; k++) 
        {
            float proj = hash_of_triangles.at(tri_i_index)[k].project_point(axe);
            min_proj_i = std::min(min_proj_i, proj);
            max_proj_i = std::max(max_proj_i, proj);
        }
        ...
        for (int k = 0; k < 3; k++) 
        {
            float proj = hash_of_triangles.at(tri_j_index)[k].project_point(axe);
            min_proj_j = std::min(min_proj_j, proj);
            max_proj_j = std::max(max_proj_j, proj);
        }
